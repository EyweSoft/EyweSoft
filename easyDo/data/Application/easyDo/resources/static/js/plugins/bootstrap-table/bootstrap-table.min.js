...
!function(a) {
    ...
    n.prototype.initContainer = function() {
    	var filterHtml = '<div class="fixed-table-filter">', fieldColumns = this.options.columns, field, fieldType;    	
    	if(fieldColumns && fieldColumns.length) {
    		filterHtml += '<div class="search-head"><div class="search-title ml10"><i class="glyphicon glyphicon-filter mr5" />'+$.i18n.prop("common.filter")+'</div><div class="search-btn"><span class="c-btn mr10 clear-all">'+$.i18n.prop("common.reset")+'</span><span class="o-btn" onclick="reLoad()">'+$.i18n.prop("common.search")+'</span></div></div>';
    		filterHtml += '<div class="from-group-a">';
    		for(var i = 0; i < fieldColumns.length; i++) {
    			field = fieldColumns[i];
    			if(!field.search) {
    				continue;
    			}
    			...
    			if(fieldType == 'TEXT') {
    				filterHtml += '<div class="from-basic">';
    				filterHtml += '   <input class="form-control" name="'+field.fields+'" id="_'+field.fields+'" type="text" placeholder="'+field.title+'" htype="'+fieldType+'">';
    				filterHtml += '</div>';
    			}if(fieldType == 'DATE' || fieldType == 'TIME') {
    				filterHtml += '<div class="from-basic include-icon singer">';
    				filterHtml += '   <input class="form-control datetimepicker" name="'+field.fields+'" id="_'+field.fields+'" type="text" placeholder="'+field.title+'" htype="'+fieldType+'">';
    				filterHtml += '   <i class="zmdi zmdi-calendar"></i>';
    				filterHtml += '</div>';
    			}else if(fieldType == 'SELECT') {
    				filterHtml += '<div class="from-basic include-icon">';
    				filterHtml += '   <input type="hidden" attrSign="_ids" id="_sexIds">';
    				filterHtml += '   <input class="form-control select gray" attrSign="_names" name="'+field.fields+'" id="_'+field.fields+'Names" type="text" placeholder="'+field.title+'" autocomplete="off" readonly="readonly" htype="'+fieldType+'">';
    				filterHtml += '   <i class="zmdi zmdi-caret-down" ></i>';
    				filterHtml += '   <div class="chosen-drop common-menu-hide">';
    				filterHtml += '   	 <ul class="chosen-results"></ul>';
    				filterHtml += '   	 <div class="chosen-all"><div class="checkbox"><input type="checkbox" name="all_'+field.fields+'" class="cp"/><label></label></div><a class="auto-link" onclick="selectConfirm($(this))">'+$.i18n.prop("common.comfirm")+'</a></div>';
    				filterHtml += '   	 <div class="no-data">'+$.i18n.prop("common.no.data")+'</div>';
    				filterHtml += '   </div>';
    				filterHtml += '</div>';
    			}else if(fieldType == 'DEPT') {
    				filterHtml += '<div class="from-basic include-icon">';
    				filterHtml += '		<input name="'+field.fields+'" id="deptIds" type="hidden" value="">';
    				filterHtml += '		<input attrsign="DEPT" autocomplete="off" class="form-control" id="deptName" name="deptName" type="text" placeholder="'+field.title+'" autocomplete="off" htype="'+fieldType+'">';
    				filterHtml += '		<i class="zmdi zmdi-search"></i>';
    				filterHtml += '</div>';
    			}
    		}
    		filterHtml += '</div>';
    ...
    n.prototype.initTable = function() {
        ...
        a.each(this.options.columns, function(c, d) {
            a.each(d, function(d, e) {
                e = a.extend({}, n.COLUMN_DEFAULTS, e),
    ...
    n.prototype.initHeader = function() {
        ...
        a.each(this.options.columns, function(f, g) {
            ...
            a.each(g, function(a, f) {
                ...
                e.push("<th" + c(' title="%s"', f.titleTooltip), f.checkbox || f.radio ? c(' class="bs-checkbox %s"', f["class"] || "") : k, c(' style="%s"', h + j), c(' rowspan="%s"', f.rowspan), c(' colspan="%s"', f.colspan), c(' data-field="%s"', f.fields), "tabindex='0'", ">"),
...
//            b.options.sortable && a(this).parent().data().sortable && b.onSort(c)
            b.options.sortable && $(this).hasClass('sortable') && b.onSort(c)
        }),
    ...
    n.prototype.initToolbar = function() {
        ...
        this.options.showFilter && g.push(c('<button class="btn btn-default' + (void 0 === this.options.iconSize ? "" : " btn-" + this.options.iconSize) + '" type="button" name="filter" title="%s">', this.options.formatFilter()), c('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.filter), "</button>"),
        ...
        a.each(this.columns, function(a, b) {
            if (!(b.radio || b.checkbox || f.options.cardView && !b.cardVisible)) {
                ...
                b.switchable && (g.push(c('<li><label><input type="checkbox" data-field="%s" value="%s"%s> %s</label></li>', b.field, a, d, b.title)),
                j++)
            }
        }),
        g.push("</ul>", "</div>")),
        ...
        this.options.showFilter && this.$toolbar.find('button[name="filter"]').off("click").on("click", function() {
        	$('.fixed-table-filter').toggle();
        }),
        ...
        j <= this.options.minimumCountColumns && b.find("input").prop("disabled", !0),
        ...
        this.options.search && (g = [],
        g.push('<div class="pull-' + this.options.searchAlign + ' search">', c('<input class="form-control' + (void 0 === this.options.iconSize ? "" : " input-" + this.options.iconSize) + '" type="text" placeholder="%s">', this.options.formatSearch()), "</div>"),
    ...
    n.prototype.initPagination = function() {
        ...
        this.options.pageNumber <= 1 && (g.addClass("disabled"),
        h.addClass("disabled")),
    ...
    n.prototype.initBody = function(b) {
        ...
        for (var k = this.pageFrom - 1; k < this.pageTo; k++) {
            var l, n = i[k], o = {}, p = [], q = "", r = {}, s = [], trId, idArray;
            if(n && n[0] && n[0].indexOf('#-%@-#@') >= 0) {
            	idArray = n[0].split('#-%@-#@');
            	n[0] = idArray[0];
            	trId = idArray[1];
            }
            ...
            g.push("<tr", c(" %s", s.join(" ")), c(' id="%s"', trId), c(' class="%s"', o.classes || (a.isArray(n) ? void 0 : n._class)), c(' data-index="%s"', k), c(' data-uniqueid="%s"', n[this.options.uniqueId]), c("%s", q), ">"),
            ...
            a.each(this.header.fields, function(b, i) {
                var j = ""
                  , l = m(n, i)
                  , q = ""
                  , r = {}
                ...
                if (x.visible) {
                    ...
                    l = h(x, f.header.formatters[b], [l, n, k], l),
                    n["_" + i + "_id"] && (s = c(' id="%s"', n["_" + i + "_id"])),
                    n["_" + i + "_class"] && (t = c(' class="%s"', n["_" + i + "_class"])),
                    n["_" + i + "_rowspan"] && (v = c(' rowspan="%s"', n["_" + i + "_rowspan"])),
                    n["_" + i + "_title"] && (w = c(' title="%s"', n["_" + i + "_title"])),
                    r = h(f.header, f.header.cellStyles[b], [l, n, k], r),
                    r.classes && (t = c(' class="%s"', r.classes)),
                    r.css) {
                        var y = [];
                        for (var z in r.css)
                            y.push(z + ": " + r.css[z]);
                    ...
                    x.checkbox || x.radio ? (q = x.checkbox ? "checkbox" : q,
                    q = x.radio ? "radio" : q,
                    j = [f.options.cardView ? '<div class="card-view">' : '<td class="bs-checkbox">', "<input" + c(' data-index="%s"', k) + c(' name="%s"', f.options.selectItemName) + c(' type="%s"', q) + c(' value="%s"', n[f.options.idField]) + c(' checked="%s"', l === !0 || l && l.checked ? "checked" : void 0) + c(' disabled="%s"', !x.checkboxEnabled || l && l.disabled ? "disabled" : void 0) + " />", f.header.formatters[b] && "string" == typeof l ? l : "", f.options.cardView ? "</div>" : "</td>"].join(""),
                    n[f.header.stateField] = l === !0 || l && l.checked) : (l = "undefined" == typeof l || null === l ? f.options.undefinedText : l,
                    j = f.options.cardView ? ['<div class="card-view">', f.options.showHeader ? c('<span class="title" %s>%s</span>', o, d(f.columns, "field", "title", i)) : "", c('<span class="value">%s</span>', l), "</div>"].join("") : [c("<td%s %s %s %s %s %s>", s, t, o, u, v, w), l, "</td>"].join(""),
                    f.options.cardView && f.options.smartDisplay && "" === l && (j = "")),
                    g.push(j)
                }
            }),
        ...
        this.$body.find("> tr[data-index] > td").off("click dblclick").on("click dblclick", function(b) {
            var d = a(this)
              ...
              , l = m(h, j);
            if (!d.find(".detail-icon").length && (f.trigger("click" === b.type ? "click-cell" : "dbl-click-cell", j, l, h, d),
            f.trigger("click" === b.type ? "click-row" : "dbl-click-row", h, g),
            "click" === b.type && f.options.clickToSelect && k.clickToSelect)) {
        ...
        this.$body.find("> tr[data-index] > td > .detail-icon").off("click").on("click", function() {
            var b = a(this)
              ...
              , e = d.data("index")
              , g = i[e];
            d.next().is("tr.detail-view") ? (b.find("i").attr("class", c("%s %s", f.options.iconsPrefix, f.options.icons.detailOpen)),
            d.next().remove(),
            f.trigger("collapse-row", e, g)) : (b.find("i").attr("class", c("%s %s", f.options.iconsPrefix, f.options.icons.detailClose)),
            d.after(c('<tr class="detail-view"><td colspan="%s">%s</td></tr>', d.find("td").length, h(f.options, f.options.detailFormatter, [e, g], ""))),
            f.trigger("expand-row", e, g, d.next().find("td"))),
            f.resetView()
        }),
    ...
    n.prototype.initServer = function(b, c) {
        ...
        f = h(this.options, this.options.queryParams, [g], f),
        a.extend(f, c || {}),
        f !== !1 && (b || this.$tableLoading.show(),
        d = a.extend({}, h(null, this.options.ajaxOptions), {
    ...
    n.prototype.resetFooter = function() {
        ...
        a.each(this.columns, function(a, f) {
            ...
            i = c("vertical-align: %s; ", f.valign),
            e.push("<td", j, c(' style="%s"', g + i), ">"),
    ...
    n.prototype.checkBy_ = function(b, d) {
        if (d.hasOwnProperty("field") && d.hasOwnProperty("values")) {
            ...
            a.each(this.options.data, function(g, h) {
                return h.hasOwnProperty(d.field) ? void (-1 !== a.inArray(h[d.field], d.values) && (e.$selectItem.filter(":enabled").filter(c('[data-index="%s"]', g)).prop("checked", b),
                h[e.header.stateField] = b,
                f.push(h),
                e.trigger(b ? "check" : "uncheck", h))) : !1
            }),
            this.updateSelected(),
    ...
    n.prototype.refreshOptions = function(b) {
        i(this.options, b, !1) || (this.options = a.extend(this.options, b),
